...
namespace CSI.Library.Security
{
    public class Authority
    {
        ...
        internal static void Authenticate(String email, String password, String ipAddress)
        {
            Storage.Users _dbUsers = new Storage.Users();
            if (!_dbUsers.Authenticate(email, password, ipAddress))
            {
                throw new UnauthorizedAccessException(Resources.Messages.AuthenticationError);
            }
        }
        internal static void CheckCompanyData(String name, String email)
        {
            String _errorMessage="";

            if (name != "")
                if (new Handlers.Companies().CheckName(name))
                    _errorMessage = Resources.Messages.AccountMessageCompanyExists;
        ...
        { SiteReader=0, SiteOperator=1, SiteManager=2 }
                
    }

    internal class Cryptography
    {
        private static string passPhrase = "PasZiv[]9";        // can be any string
        private static string saltValue = "s@1tV@lu3";        // can be any string

...
{
    public partial class MeterFuelLoadAdd : BasePage
    {
        private Library.Objects.Sites.Meters.FuelMeter _Meter;
        private List<Library.Objects.Sites.Meters.Series.FuelData> _Data;
        ...
        private void SetMenu()
        {
            ...
            ((Main)Page.Master).MenuNavigation.Initialize(_site.IdSite, _site.Title, _Meter.IdMeter, _Meter.Identification, Console.Controls.ucMenuNavigation.MeterType.Fuel, Console.Controls.ucMenuNavigation.MenuItem.MeterLoad, WebUI.Common.GetPermissionFromContext(I,_site));
        ...
        private void LoadTexts()
        {
            ...
            lblGridHeaderType.Text = Resources.Data.FuelType;
            ...
            lnkLoadSetGrid.Text = Resources.Data.Add;
            btnSave.Text = Resources.Data.Save;

        }
        private void LoadData()
        {
            _Data = new List<Library.Objects.Sites.Meters.Series.FuelData>();
            ...
            foreach (RepeaterItem _item in rptGrid.Items)
            {
                if (_item.ItemType == ListItemType.AlternatingItem || _item.ItemType == ListItemType.Item)
                {
                    ...
                    Library.Objects.Auxiliaries.Types.FuelType _type = I.GetFuelType(Convert.ToInt64(((HiddenField)_item.FindControl("hdnGridType")).Value));
                    Library.Objects.Auxiliaries.Units.Unit _unit = I.GetUnit(Convert.ToInt64(((HiddenField)_item.FindControl("hdnGridUnit")).Value));

                    Library.Objects.Sites.Meters.Series.FuelData _fuelData = new Library.Objects.Sites.Meters.Series.FuelData(_index, _date, _type, _value, _unit);
                    _Data.Add(_fuelData);
                }
            }
        }
        ...
        void rptGrid_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                Library.Objects.Sites.Meters.Series.FuelData _data = (Library.Objects.Sites.Meters.Series.FuelData)e.Item.DataItem;
        ...
        {
            Library.Objects.Sites.Meters.Series.FuelData _fuelData = new Library.Objects.Sites.Meters.Series.FuelData(_Data.Count, Convert.ToDateTime(hdnLoadDate.Value), I.GetFuelType(Convert.ToInt64(ddlLoadTypes.SelectedValue)), Convert.ToDouble(txtLoadValue.Text), I.GetUnit(Convert.ToInt64(ddlLoadUnits.SelectedValue)));
            _Data.Add(_fuelData);
